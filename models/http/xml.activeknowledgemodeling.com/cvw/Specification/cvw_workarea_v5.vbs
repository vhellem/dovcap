...
'-----------------------------------------------------------
    Private Sub createWorkareaWindow(compWorkareaWindow)
        ...
        call connectContentSearchModel()
    End Sub
...
'-----------------------------------------------------------
    Private Sub connectContentSearchModel()
        dim specModel
        dim obj, rel
        dim specObjView
        dim context

        if Len(ContentSearchModel) > 0 then
            set specModel = metis.findInstance(ContentSearchModel)
            if isEnabled(specModel) then
                set specObjView = specModel.views(1)
                if isEnabled(work_window) then
                    ' Connect rel from work_window to model context specification container
                    set obj = work_window.instance
                    set rel = currentModel.newRelationship(hasSearchSpecificationType, obj, specModel)
                end if
            end if
        end if
    End Sub
