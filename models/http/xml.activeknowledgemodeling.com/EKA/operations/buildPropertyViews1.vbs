    ...
    'stop
            ...
            set instView = viewExists(inst, modelView)
        end if
        if isValid(instView) then
            call buildPropertyViews(instView)
        end if
    end if
    ' End

    private sub buildPropertyViews(objView)

        dim obj, propList, prop, propView, rel, rels
        ...
        for each propView in propList
            set pv = cvwObjView.create(objView, objView, propView, -1)
        next
    end sub
