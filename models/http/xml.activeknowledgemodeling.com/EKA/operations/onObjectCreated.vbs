...
if isValid(parentInst.ownerModel) then
    ...
    if isEnabled(modelObject) then
        ...
        if isModel then
            if modelObject.uri <> model.uri then
                ...
                'set inst.parent = modelObject
            end if
        else
            set inst = relocate(inst, modelObject, instView)
            'set inst.parent = modelObject
            if instView.parent.uri = wObjectView.uri then
                textscale = wObjectView.textscale
                if instView.isNested then
                    instView.close
                    instView.textscale = textscale * 1.75
                end if
            end if
        end if
    end if
