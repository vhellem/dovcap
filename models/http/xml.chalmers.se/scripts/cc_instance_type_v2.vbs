        ...
        if isEnabled(instTypeModel) and isEnabled(instModel) then
            ...
            if isEnabled(instanceType) then
                ...
                for each inst in instances
                    removed = false
                    if not inst.type.inherits(instanceType) then
...
'-----------------------------------------------------------
    Public Function newInstance(compObj, instanceType, instName)
        ...
        if isEnabled(instTypeModel) and isEnabled(instModel) then
            ...
            '    call copyProperties(compObj, instanceType, instModel)
            'end if
            if isEnabled(instanceType) then
