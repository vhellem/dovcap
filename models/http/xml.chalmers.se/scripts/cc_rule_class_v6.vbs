...
'-----------------------------------------------------------
    Public Sub startConfigureCC(obj1, variantName, productType, ccModel)
        ...
        if Len(variantName) = 0 then
            variantName = obj1.title
        end if
        if not isEnabled(ccModel) then set ccModel = contentModel
        ...
        if not isEnabled(modelObject) then
            ...
            set modelObject = ccModel.newObject(modelObjectType)
                    ...
                    if includedInConfig(ceRel) then
                        ...
                        if includedInConfig(ceObj) then
                            ...
                            if isEnabled(part) then
                                ...
                                set rel    = ccModel.newRelationship(memberType, ccPart, part)
...
'-----------------------------------------------------------
    Private Sub createPartProps(obj, model, modelObject, ccPart)
        ...
        set ppObjects    = obj.getNeighbourObjects(0, hasPpType, ppType)
