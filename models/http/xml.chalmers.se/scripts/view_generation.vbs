...
public workplaceType, leftPaneType, rightPaneType, workareaType, contextType, contextObject, actionType, valueType, baseRelType, usesCcType
...
dim InputRuleMethod, InputViewstyle, InputCcViewstyle, InputNeighbourViewstyle
dim InputUsesCcType, InputConfigure
dim method, apply_rule, value, test
...
InputUsesCcType       = "http://xml.chalmers.se/class/uses_configurable_component.kmd#uses_configurable_component"
...
InputViewstyle        = Split(InputArray(9), "=", -1, 1)(1)
...
set usesCcType        = metis.findType(InputUsesCcType)
...
' Set viewstyle
if Len(InputViewstyle) > 0 then
    modelView.setViewstyle(InputViewstyle)
end if
...
'stop
    if isEnabled(criteria) then
        ...
        for each inst in instances
            removed = false
            if not inst.ownerModel.uri = modelObj.uri then
                    ...
                    for each rel in rels
                        if isEnabled(topObjectRelType) then
                            if rel.type.inherits(topObjectRelType) then
                                if apply_rule then
                                    on error resume next
                                    set prop = rel.type.getProperty(RuleEvaluatedToProperty)
                                    if isEnabled(prop) then
                                        isInConfig = rel.getValue(prop).getInteger
                                    else
                                        isInConfig = 1
                                    end if
                                else
                                ...
                                if isInConfig = 1 and rel.target.uri = inst.uri then
                                    isTop = true
                                end if
                            end if
                        end if
    ...
    if isEnabled(criteria) then
        ...
        if isEnabled(topObjectRelType) then
            ...
            for each inst in instances
                ...
                set rels = inst.neighbourRelationships
                for each rel in rels
                    if rel.type.uri = topObjectRelType.uri then
                        instances.removeAt(i)
                        removed = true
                        exit for
                    end if
...
'    end function
    ...
    function createTreeView(obj, isTop, instances, parentView, modelType, viewStrategy, apply_rule)
        ...
        dim instList, objType, relType, relDir, rel, relList
        dim doIt, test, textScale, parentAbsScale, objAbsScale
        dim partOfRules, typeUri, value
        dim part, parts
        dim prop, rule, ruleEngine
        dim isInConfig
'stop
        set objView = parentView.newObjectView(obj)
        if not objView.isNested then
            if isTop then
                objView.textScale = 0.5
            else
        ...
        objAbsScale = objView.absTextScale
...
'stop
            ...
            for each rule in partOfRules
                if StrComp(rule.type.name,"partOfRule") = 0 then
                    relDir = 0
                    set value = rule.getNamedValue("RelDir")
                    if value.isValid then
                        relDir = value.getInteger
                    end if
                ...
                if isEnabled(obj) then
                    ' Get neighbours
                    set relList = obj.getNeighbourRelationships(relDir, relType)
...
'stop
        ...
        objView.absTextScale = objAbsScale
    ...
    sub createObjectViews(isTop, instances, parentView, modelType, viewStrategy, apply_rule)
        ...
        for each inst in instances
            ...
            if rels.count = 0 then
                'call configureObject(inst, viewStrategy)
...
'stop
	...
	'-----------------------------------------------------------------------------------------------------
	' [5l] Function&Sub - includedInConfig
	'-----------------------------------------------------------------------------------------------------
    function includedInConfig(rel, RuleEngineProperty, RuleCodeProperty, apply_rule)
        dim prop
        dim ruleEngine, rule
        ...
        if apply_rule = 0 then exit function
...
'stop
                            ...
                            includedInConfig = rel.getNamedValue(RuleEvaluatedToProperty).getInteger
                        end if
            end select
        end if
    ...
    function getMCTinstances(startObject, criteria, filter)
        ...
        if isEnabled(criteria) then
            ...
            if not isEnabled(startObject) then
                ...
                set instances = metis.runCriteriaOnInstance(criteria, startObject)
            end if
            i = 1
            for each inst in instances
                if not inst.ownerModel.uri = modelObj.uri then
...
'stop
        if isEnabled(topObject) then
            'call configureObject(topObject, viewStrategy)
