...
'-----------------------------------------------------------
    Public Function configureCC(obj)

        set configureCC = Nothing
        call configureFunctionMeans(obj)
        call configureTopCC(obj)
        set configureCC =  createInstantiatedPartStructure(obj)
    End Function

'-----------------------------------------------------------
    Public Function createInstantiatedPartStructure(ccObj)
        dim modelObj, modelObject, ccPart
        dim product, products
        dim parts

        set createInstantiatedPartStructure = Nothing
        if isEnabled(ccObj) then
            ...
            if isEnabled(modelObject) then
                ...
                call createPartStructure(modelObject, ccPart, ccObj)
            end if
            set createInstantiatedPartStructure = ccPart
        end if
    End Function
                                ...
                                if includedInConfig(crRel) then
                                    ...
                                    for each ccRel in ccRels
                                        if includedInConfig(ccRel) then
                                            ...
                                            ' CC found - create and connect the new part
                                            'set part   = modelObject.newPart(partType)
                                            'if isEnabled(part) then
                                            '    part.title = obj.title
                                            '   Create sub-structure
                                            '   call createPartStructure(modelObject, part, obj)
                                            'end if
                                            set part = configureCC(obj)
                                            set rel  = contentModel.newRelationship(memberType, ccPart, part)
                                        end if
                                    next
                                end if
            ...
            if isEnabled(frObj) then
                ...
                if primary > 0 then
                    ...
                    for each dsObj in dsObjects
                        ' Find parameters
                        set valueObjects = dsObj.getNeighbourObjects(0, hasValueType, valueType)
                        for each valueObj in valueObjects
                            ...
                            for each defObj in defObjects
                                propName = defObj.title
                                exit for
                            next
                            ' Create property
                            set prop = modelObject.newPart(propertyType)
                            if isEnabled(prop) then
                                if isEnabled(defObj) then
                                    prop.title = defObj.title
...
'-----------------------------------------------------------
    ...
    Private Sub configureCR(relship)
        ...
        for each rel in rels
            call configureSubCC(rel)
        next
    End Sub

    Private Sub configureSubCC(relship)
        dim obj
...
'-----------------------------------------------------------
    Public Sub configureFunctionMeans(ccObj)
        dim obj, objects
        dim rels

        ' Find top FR's
        set objects = ccObj.getNeighbourObjects(0, explainsType, frType)
    ...
    Private Sub configureFrDsC(frObj)
        dim obj, objects
                 ...
                 if isEnabled(rel) then
                    ...
                    if isEnabled(dsObj) then
                        ...
                        for each obj in objects
                            ...
                            if isEnabled(obj) then
                                ' Configure C
                                call configureInstance(obj)
                            end if
                        next
                        ' Configure DS
                        call configureInstance(dsObj)
                    end if
                end if
            next
...
'-----------------------------------------------------------
    Private Sub configureInstance(inst)
        call transformRulesToScripts(inst)
        call ccRuleEngine.executeRules(inst)
    End Sub
